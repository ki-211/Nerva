"""Run a disposable synthetic image -> OCR -> draft -> approval smoke."""
...
from apps.api.app.prompts import OCR_PROMPT_VERSION
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="nerva-real-image-smoke-") as tempdir:
        ...
        try:
            ...
            recognized = ocr.recognize(image_data_url(temporary_image), source_id=source["id"], sequence=1)
            image_path.unlink()
            ...
            candidates = retrieve_candidates(combined, source["title"], store.list_documents(user["id"]), limit=8)
            ...
            print({
                "ocr_model": ocr.model, "text_model": ai.model,
                "image_tokens": recognized.image_tokens,
                "knowledge_units": len(extraction.units), "changes": len(proposals),
